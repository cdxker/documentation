...
Mintlify's AI features have different access patterns when authentication is enabled:

- **AI assistant and Slack bot**: Work with both public and private documentation. These features can access and search all content that your organization has access to, regardless of authentication settings.
- **MCP server**: Only works with publicly accessible documentation. The MCP server cannot access content behind authentication.
