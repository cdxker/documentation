...
The Slack app adds a bot named `@mintlify` to your Slack workspace that can search your documentation and answer users' questions. The bot responds to direct messages from workspace members, @mentions, and to any questions in a channel specifically named `#ask-ai`.

<Note>
  External users (guests) may not be able to send direct messages to the bot. If you're working with external users, have them @mention the bot in shared channels instead.
</Note>
